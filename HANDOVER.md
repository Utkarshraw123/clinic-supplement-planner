...
- **Repo:** `/Users/utkarshrawat/Wild Dash/supplement-selection-db` (holds `.git`; the parent `Wild Dash/` is NOT a repo). Branch **`main`**, HEAD **`cd47be1`**.
- **Guide "Where to buy" links (2026-08-26, HEAD cd47be1, 70 tests):** the Supplement Instruction Guide PDF now renders a **"Where to buy your supplements"** section — one clickable PDF hyperlink per prescribed product, built in `lib/delivery.ts` from `plan.items[].product.suppliers[0].url` and passed to `buildGuidePdfData(patient, guide, links)` → `lib/pdf.tsx` (new `GuideLink` type, `@react-pdf` `Link`, host shown as visible text). All 44 WN products already have their wildnutrition.com supplier link (seeded), so coverage is complete. Verified by rendering a sample PDF via a temp vitest → Read the PDF (the only way to eyeball @react-pdf output). Prepare page notes links are auto-included.
