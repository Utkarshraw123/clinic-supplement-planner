...
**Read `SESSION_HANDOFF_2026-09-01.md` FIRST** (current state + the exact next step: resume the Cytoplan scrape, E2), then this file for deep reference. Last major refresh **2026-08-27**; session handoff appended **2026-09-01**.
...
- **Repo:** `/Users/utkarshrawat/Wild Dash/supplement-selection-db` (holds `.git`; the parent `Wild Dash/` is NOT a repo). Branch **`main`**, HEAD **`c15e0ff`** (clean tree, 93 tests, `npm run build` clean). **▶ Resume point + full current state: `SESSION_HANDOFF_2026-09-01.md`.**
- **▶ FEEDBACK ROADMAP in progress (2026-09-01):** 12 feedback items across 5 workstreams. Roadmap artifact: **claude.ai/code/artifact/3e54b0ab-1c68-446e-b452-dd73596cf7d9** (decisions all taken = my recommendations). Spec: `docs/superpowers/specs/2026-09-01-duplicate-plan-and-drafts-housekeeping-design.md` (wrong file — the feedback spec was NOT written; roadmap artifact is the plan of record). **WORKSTREAM A DONE + deployed (HEAD 7e037ec) except A6 Merriweather font:** the prescription PDF's supplement section is now a **structured TABLE built from the plan items** (`buildSupplementRows` in `lib/guide.ts`), NOT the old editable free-text — this **fixed the "5 added → 3 printed" bug** at root (two causes: stale saved `supplement_text`, and `wrap={false}` clipping overflow; both gone — table wraps across pages, verified 5 items → 5 printed). Delivers: table (Supplement/Dosage/Where-to-buy), **brand + pack size** per line (new `plan_items.size` + `SIZE_OPTIONS`), **all vendor buy-links** (not just first), **per-brand promo codes** (new `brands.promo_code`, editable on `/catalog/brands`; per-item order code overrides), **marketing descriptions removed**, **closing Notes block** (new `plan_guide.notes`), and the **note-preset dropdown** on each plan item (Workstream C, from `note_snippets` category 'supplement'). Prepare page now shows a READ-ONLY supplement preview (details edited in the builder). `ProductDetail` gained `brand_promo_code`. New cols migrated on prod. **NOW 11 of 12 items DONE + deployed (HEAD 3bc32f7, 93 tests):** A (incl. **A6 Merriweather** — `lib/pdf-fonts.ts` embeds 400/700 as base64 WOFF data URIs, `Font.register`, no runtime fetch/tracing), B (**ProtocolSearch** on `/protocols`, **AddProductSearch** in the plan builder), C (note-preset dropdown), D (**vegan-marine** in `lib/flagging.ts` — marine=fish/omega-3/collagen accepted for "Vegan (marine OK)"; **medications** seeded via `scripts/seed-clinical-terms.ts` = "Vegan (marine OK)" diet + ~20 meds incl. Levothyroxine, run local+prod), E1 (**retired Alternative-formats** UI in catalog editor + plan builder; DB cols left dormant). **ONLY E2 (#6 Cytoplan import) REMAINS — BLOCKED/needs a decision:** cytoplan.co.uk is **Magento** (no products.json) and 403s automated fetch (bot-blocked); browseable in the browser pane but a full import = scraping hundreds of Magento pages, and bulk-loading a NEW brand's auto-derived allergen tags live is a clinical-safety risk (needs Lorna sign-off, Decision 5). RECOMMENDED PATH: build `scripts/seed-cytoplan.ts` + `scripts/data/cytoplan.json` (Wild-Nutrition seed pattern) and populate a CURATED set of the Cytoplan products Lorna actually prescribes (get her list) rather than a fragile whole-catalogue scrape. Cytoplan brand already exists on prod; set its `promo_code` on `/catalog/brands`. Dead code from E1 (chooseAlternativeAction, setItemAlternative, addAlternativeAction, product_alternatives table) can be pruned later.
- **Demo login lorna123 (2026-09-01, HEAD 9f69ebd, pushed):** the login now accepts a **username OR email** (field relabelled "Email or username", input `type=text`; the API already matches on the `users.email` column so a username value just works — no auth-logic change). `scripts/seed-demo-login.ts` (idempotent) sets the clinic admin's login to **`lorna123` / `lorna123`** (renamed admin id 1, was `admin@clinic.test`). Password check is UNCHANGED — a wrong password still 401s (this is NOT accept-any-password; user was offered that and chose the safe fixed password). Team inline-edit email field relaxed to `type=text` so username logins stay editable. **Weak demo password by design → change at /account before real patient data.** Deploy order was code-first (username login is backward-compatible), then run the seed on prod. **NEW PROD LOGIN: `lorna123` / `lorna123`** (supersedes admin@clinic.test / Clinic-bfde5f11-3b578f24).
- **Duplicate plan + draft-plans housekeeping (2026-09-01, HEAD 5339dfe, 90 tests, pushed):** two features. (1) **Duplicate a plan (same patient)** — `lib/plans.ts duplicatePlan(sourcePlanId, authorId)` copies every item (product, dosing, duration, order_code, note, alternative, position) into a NEW draft for the same patient (source untouched); a **"Duplicate → new draft"** button on the patient History page (per finalised plan; `listSnapshotsForPatient` now returns `plan_id`) and on the drafts list → lands in the builder on the copy to tweak & re-send. (2) **Draft-plans housekeeping** — the dashboard **"Draft plans"** stat card is now a LINK to a new **`/plans/drafts`** worklist (patient · items · last-updated · built-by) with per-row **Open & send** / **Duplicate** / **Delete** (`DeleteDraftButton`, native confirm; `deletePlanAction`→`deletePlan` batch-removes items+guide+plan, leaves patient/snapshots). **Model change (the hinge):** patients may now have MULTIPLE drafts; the builder route accepts **`?plan=<id>`** (`resolveDraftPlanId` validates it's a draft of that patient, else falls back to `getOrCreateDraftPlan`). No new columns → no prod migration. Spec: `docs/superpowers/specs/2026-09-01-duplicate-plan-and-drafts-housekeeping-design.md`. Verified locally end-to-end (duplicate copies all fields, open-specific-draft, delete drops the count).
- **Pre-demo QA sweep + ProductSearch fix (2026-08-27, HEAD d6f146f, LIVE on prod):** functional-tested the demo's headline flows on prod, all green — **allergen hard-block** (Amelia Hughes + Immune Support → "Blocked: contains mushroom", banner, Finalise hidden, allergy-safe suggestions shown), **protocol apply** (Menopause Support → 4 items + flagging recompute), **catalog search API** (correct), **CSV exports** (patients/products/analytics all 200), **analytics** (renamed staff, correct counts), all 14 routes 200. Test items added during QA were removed (Amelia, Matilda clean). **Bug found + fixed:** `components/ProductSearch.tsx` didn't guard against out-of-order fetch responses (fast typing could show stale results) — added an `active` flag per effect run. A transient server-components 500 seen mid-QA was just the deploy-swap window (cleared on reload; logs show requests completing clean). **Demo run-of-show artifact (keep-open reference for the live demo): https://claude.ai/code/artifact/f33a67f4-7d9c-418a-8c1d-7542ecf17492** — patient cheat-sheet + 6-step click-path + principles + go-live checklist.
- **Comprehensive demo send + PDF-route tracing fix (2026-08-27, HEAD 9262273, LIVE on prod):** built a maximally-complete example prescription for **Rebecca Nolan (id 15)** — 4 items (Perimenopause Complex, Iron Plus, Magnesium, Vitamin D) each with custom dosing + duration + `order code: LORNA10` + a per-item note, **Vitamin D carrying an alternative format** (linked Vitamin D↔Vitamin D Gummies in the catalogue first, since no WN products had alternatives), full guide (intro/next/lifestyle/dietary), meds auto-filled (Anaemia), then **finalise-&-sent to `utkarshrawatofficial@gmail.com`** (the Resend account-owner address the `onboarding@resend.dev` test sender actually delivers to) → snapshot 8, valid 108KB PDF, history shows "Sent · Emailed". **FIX:** the logs showed an Unhandled Rejection `pdfkit/js/standard-fonts/Helvetica.cjs` on `GET /api/snapshots/[id]/pdf` — that route imports `lib/delivery` (→ @react-pdf/pdfkit) but was NOT in `next.config.mjs outputFileTracingIncludes`; added it (verified `standard-fonts` now in its `.nft.json`). PDF retrieval worked anyway (base64 read), but this makes it bundle-complete + quiet.
- **Edit-user + demo-ready prep (2026-08-27, HEAD 584a46d, 86 tests, LIVE on prod):** the Team page had add/remove but NO edit — added inline **edit-user** (`lib/users.ts updateUser(id,{name,email,role})` + test; `updateUserAction` with a self-lockout guard so an admin can't demote/remove their own account). `app/admin/users/page.tsx` rebuilt into labelled inline-edit rows (`.user-row`/`.user-edit` CSS). **Demo prep done on PROD via the app:** renamed placeholder staff **Member 1→Hannah Reeves (hannah@clinic.test)** and **Member 2→Nadia Osei (nadia@clinic.test)**; fixed patient **"Lucy francis"→"Lucy Francis"** (id 17); built a **flagship finalised plan** for **Charlotte Bennett (id 18)** — Menopause Complex + Magnesium + Vitamin D with dosing, the new duration + `order code: LORNA10`, full guide (intro/lifestyle/dietary/next), finalised download-only → valid 109KB branded PDF (`/api/snapshots/7/pdf`). Smoke-tested all 14 prod routes = 200 + authed. **OPEN demo blemish:** the only two "sent" snapshots (Aisha Khan id 12) were emailed to `utkarshrawatofficial@gmail.com` (dev test address) and show on the dashboard "Recently sent" — no per-snapshot delete exists (only whole-patient erasure); to clean, delete Aisha via Danger Zone then re-run `scripts/seed-uat-patients.ts` on prod to restore her fresh. Left for the user to OK (destructive).
