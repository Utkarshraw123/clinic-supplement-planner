...
- **Repo:** `/Users/utkarshrawat/Wild Dash/supplement-selection-db` (holds `.git`; the parent `Wild Dash/` is NOT a repo). Branch **`main`**, HEAD **`b9e57d5`** (clean tree, 79 tests, `npm run build` clean).
...
- **▶ NEXT UP — GO-LIVE items only (see §9):** verify a Resend sending domain (email_from is still the test sender `onboarding@resend.dev`), change the prod admin password (still the deploy-generated `Clinic-bfde5f11-3b578f24`), Lorna's clinical sign-off on the auto-derived allergen tags, real staff/Lorna accounts (currently `admin@clinic.test` + placeholder Member 1/2), and clear all UAT/test data via the app's admin patient-erasure (Danger Zone) before real clients — NOT SQL.
