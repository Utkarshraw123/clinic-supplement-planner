...
- **Repo:** `/Users/utkarshrawat/Wild Dash/supplement-selection-db` (holds `.git`; the parent `Wild Dash/` is NOT a repo). Branch **`main`**, HEAD **`6390544`**.
...
- **State:** **all 5 planned phases + a 6-item enhancement round built, tested, merged, pushed AND DEPLOYED TO PRODUCTION. 62/62 vitest tests pass. `npm run build` clean (27 routes).**
...
- **Product form fields (commit 6390544, 2026-08-26):** the add/edit product form now captures name, **description** (`products.description` — internal catalog reference, NOT on the client PDF), brand (a **type-or-pick datalist** — typing a new brand creates it via `findOrCreateBrand`, case-insensitive; `resolveBrandId` in actions prefers `brandName` over `brandId`), package size, form, product link (scan → supplier link), allergen/other tags, and Standard note. `ensureColumn` adds `products.description` to existing DBs.
...
npx vitest run                       # 62 tests, keep green
