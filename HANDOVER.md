...
- **Repo:** `/Users/utkarshrawat/Wild Dash/supplement-selection-db` (holds `.git`; the parent `Wild Dash/` is NOT a repo). Branch **`main`**, HEAD **`b6a67da`** (clean tree, 74 tests, `npm run build` clean).
- **▶ NEXT UP (requested, NOT yet done — start here):** the last request was **"Add 15 more patients and add a search bar"** — it was interrupted before any work. TODO: (1) add ~15 more UAT patients (extend `scripts/seed-uat-patients.ts` with new names/attrs, run on local + prod); (2) add a **patient search bar** on `/patients` — the list at `app/patients/page.tsx` is server-rendered; simplest is a small client component that filters the loaded list by name/DOB (pattern to copy: `components/ProductSearch.tsx` — the catalog's debounced client search). Then the open GO-LIVE items still stand (see §9): verify a Resend sending domain (email_from is still the test sender `onboarding@resend.dev`), change the prod admin password, Lorna's clinical sign-off on the auto-derived allergen tags, and real staff/Lorna accounts (currently `admin@clinic.test` + placeholder Member 1/2).
